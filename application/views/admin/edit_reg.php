<div id="page">
<?php if($no_user) { ?> No user found! <?php } else { 
...
<h1 class="heading">Edit an existing registration</h1>
...
<?php if($this->input->get("confirm") =="true") { ?>
<div class="done-msg">
The registration for <?php echo $user[0]["name"];?> has been confirmed.
</div>
<?php }?>

<div style="background:#e1dddd;padding: 10px;font-weight:bold;border:1px solid #cac6c5;">
Registration fee calculated :- <?php echo $total_cash; ?>Rs
</div>

<div>
	<label for="bid">Bodhi id</label><input type="text" name="username" value="<?php echo $this->input->post('username'); if(!$_POST && $user[0]["username"]) { echo $user[0]["username"]; } ?>">
</div>
<div>
	<label for="name">Name</label><input type="text" name="name" value="<?php echo $this->input->post('name'); if(!$_POST && $user[0]["name"]) { echo $user[0]["name"]; } ?>">
</div>
...
"Zamorins Guruvayurappan College, Kozhikode" => "Zamorins Guruvayurappan College, Kozhikode"
	...
	?>
<input id="college" type="text" name="college" value="<?php echo $this->input->post('college'); if(!$_POST && $user[0]["college"]) { echo $user[0]["college"]; } ?>" placeholder="Enter your college">
	<script>
...
<div>
	<label for="cno">Contact Number</label><input type="text" name="cno" value="<?php echo $this->input->post('cno'); if(!$_POST && $user[0]["cno"]) { echo $user[0]["cno"]; } ?>">
</div>

<div>
	<label for="ce">Contact Email</label><input type="text" name="ce" value="<?php echo $this->input->post('ce'); if(!$_POST && $user[0]["email"]) { echo $user[0]["email"]; } ?>">
</div>

<div>
	<label for="confirm">Registration Confirmation</label><?php if($user[0]["confirm"] == 0) { echo '<div style="display:inline-block;background:#efded9;padding:10px;color:#b77767;margin-left:10px;">The candidate hasn\'t confirmed his/her identity and paid the regsiatration fee.</div>';}else{
	echo '<div style="display:inline-block;background:#c2e4b7;padding:10px;color:#377923;margin-left:10px;">The candidate has confirmed his/her identity and paid the regsiatration fee.</div>';
	} ?>
</div>
...
<div class="labels">
	...
	<?php $radio_is_checked = $this->input->post('dept') === 'ec'; if(!$_POST && $user[0]["dept"] == "EC") { $radio_is_checked = true; } echo form_radio('dept', 'ec', $radio_is_checked); echo form_label('EC', 'ec');?> 
	<?php $radio_is_checked = $this->input->post('dept') === 'cs'; if(!$_POST && $user[0]["dept"] == "CS") { $radio_is_checked = true; } echo form_radio('dept', 'cs', $radio_is_checked); echo form_label('CS', 'cs');?> 
	<?php $radio_is_checked = $this->input->post('dept') === 'ce'; if(!$_POST && $user[0]["dept"] == "CE") { $radio_is_checked = true; } echo form_radio('dept', 'ce', $radio_is_checked); echo form_label('CE', 'ce');?> 
	<?php $radio_is_checked = $this->input->post('dept') === 'me'; if(!$_POST && $user[0]["dept"] == "ME") { $radio_is_checked = true; } echo form_radio('dept', 'me', $radio_is_checked); echo form_label('ME', 'me');?> 
	<?php $radio_is_checked = $this->input->post('dept') === 'eee'; if(!$_POST && $user[0]["dept"] == "EEE") { $radio_is_checked = true; } echo form_radio('dept', 'eee', $radio_is_checked); echo form_label('EEE', 'eee');?> 
	<?php $radio_is_checked = $this->input->post('dept') === 'it'; if(!$_POST && $user[0]["dept"] == "IT") { $radio_is_checked = true; } echo form_radio('dept', 'it', $radio_is_checked); echo form_label('IT', 'it');?> 
	<br><label for="dept" style="width:24%;"></label><span style="margin-left:4px;">
	<?php $radio_is_checked = $this->input->post('dept-other') === 'other'; if(!$_POST && !in_array($user[0]["dept"], array('EC','CS','CE','ME','EEE','IT'))) { $radio_is_checked = true; } echo form_radio('dept-other', 'other', $radio_is_checked); echo form_label('other', 'other');?> 
	<input type="text" name="dept-text" id="dept-text" value="<?php if($this->input->post('dept-other') == "other"){ echo $this->input->post('dept-text'); }  if(!$_POST && !in_array($user[0]["dept"], array('EC','CS','CE','ME','EEE','IT'))) { echo $user[0]["dept"]; } ?>" maxlength="30" style="display:none;margin-left:10px;width:140px;" title="specify your department here">
	</span>	<script>
...
if($v->stopped!=1) {
$ch = false; 
if(is_int(array_search((string)$v->event_id, $peve)) OR check_registered($v->event_id, $reg_for_events)   )
{
$ch = true;
} 
$data = array(
...
<div id="tab-container" class="tab-container">
 ...
 <h4><?php echo $tab_heads[$i]; ?></h4> <?php echo $tab_bodies[$i]; if(count($closed_events[$i])>0) {  ?>
...
<div class="last" style="margin-top: 0;text-align:center;">
 <?php if($user[0]["confirm"] == 0) { ?>
<a id="justconfirm" style="border: solid rgb(160, 161, 160) 1px;background: buttonface;padding: 6px;font-family: sans-serif;font-size: 12px;text-decoration: none;color: black;font-weight: normal;" href="<?php echo base_url(); ?>admin/confirm-reg/<?php echo $user[0]["user_id"]; ?>">Confirm Registration</a>
 <?php } ?>
	
	<input type="submit"  <?php if($user[0]["confirm"] == 0) { ?> style="display:none;" <?php } ?> name="submit" class="button" id="sub-but" value="Save changes <?php if($user[0]["confirm"] == 0) { ?> & Confirm Registration <?php } ?>"> 	
	<br><br> <b>OR</b> <br><br> <a style="color:red;" href="<?php echo base_url(); ?>admin/delete-reg/<?php echo $user[0]["user_id"]; ?>">Delete this Registration</a>
	</div>
</form>
 <?php if($user[0]["confirm"] == 0) { ?>
<script>
