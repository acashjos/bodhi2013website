...
$db = 'bodhi_AAA2k13';
  $link = mysql_connect( $host  , $user, $pw ) ;
       ...
       $q="select events.event_name as event,count(`event_regs`.reg_id) as num from `event_regs`,`events`  where  `event_regs`.event=events.event_id group by `event_regs`.event order by num desc";

 $query = "(select 0,count(user_id) as num from `online_regs` ) 
 union ($q limit 0,5 ) 
 union (SELECT name as event,college as num FROM `online_regs`  order by user_id desc limit 0,5)
 union (select college as event , count(*) as num from `online_regs` where LOWER(college) not like '%viswajyothi%'  group by college order by num desc limit 0,5)
...
<table>
<tr class='titlebar'><td id='all' rowspan='6'><h1>Total Registrations</h1><div><?php echo $e[0][1];?></div></td>
